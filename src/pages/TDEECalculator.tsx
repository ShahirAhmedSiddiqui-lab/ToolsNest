...
export const TDEECalculator = () => {
  ...
  const protein = (tdee * pPct) / 4;
  const carbs = (tdee * cPct) / 4;
  const fats = (tdee * fPct) / 9;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-surface-container-lowest border border-border-slate rounded-xl p-6 lg:p-8 space-y-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-heading-navy mb-2">Diet Preference (for Macros)</label>
          ...
          <div className="lg:col-span-5">
            <div className="bg-surface-container-lowest border border-border-slate rounded-xl p-6 lg:p-8 sticky top-24">
              <h2 className="text-xl font-bold text-heading-navy mb-6 text-center">Your TDEE Result</h2>
              
              <div className="text-center mb-8">
                <div className="text-sm font-medium text-on-surface-variant uppercase tracking-wider mb-2">Maintenance Calories</div>
                <div className="text-5xl font-bold text-primary mb-2">
                  {bmr > 0 ? Math.max(0, Math.round(tdee)).toLocaleString() : "0"}
                </div>
                <div className="text-on-surface-variant text-sm">kcal / day</div>
              </div>

              <div className="space-y-4">
                <div className="text-sm font-bold text-heading-navy uppercase tracking-wider mb-2">Maintenance Macronutrients</div>
              ...
              <div className="mt-8 pt-6 border-t border-border-slate text-center text-xs text-on-surface-variant space-y-2">
                <p>Basal Metabolic Rate (BMR): {bmr > 0 ? Math.round(bmr).toLocaleString() : 0} kcal/day</p>
                <p>TDEE includes your BMR multiplied by your activity level factor ({activityLevel}).</p>
              </div>
            </div>
          </div>
        </div>
