...
const tools = [
  { file: 'EmailWriter.tsx', icon: 'Mail', title: 'Email Writer', desc: 'Write professional emails with AI assistance.', btn: 'Generate Email', inputPlaceholder: 'What should the email be about? (e.g. Asking for a sick leave)', outputPlaceholder: 'Generated email will appear here...' },
  { file: 'TextRewriter.tsx', icon: 'Type', title: 'Text Rewriter', desc: 'Rewrite and improve your text instantly.', btn: 'Rewrite Text', inputPlaceholder: 'Paste your text here...', outputPlaceholder: 'Rewritten text will appear here...' },
  { file: 'GrammarFixer.tsx', icon: 'CheckCircle', title: 'Grammar Fixer', desc: 'Fix grammar and spelling mistakes.', btn: 'Fix Grammar', inputPlaceholder: 'Paste your text with errors...', outputPlaceholder: 'Corrected text will appear here...' },
  { file: 'QuizGenerator.tsx', icon: 'Brain', title: 'Quiz Generator', desc: 'Generate a quiz from your notes.', btn: 'Generate Quiz', inputPlaceholder: 'Paste your study notes here...', outputPlaceholder: 'Generated quiz questions will appear here...' }
];

const dir = 'src/pages';

const template = (tool) => `import { useState } from "react";
import { ${tool.icon}, Play, Copy, CheckCircle2, Trash2 } from "lucide-react";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 flex-1 min-h-[500px]">
          ...
          <div className="flex flex-col bg-surface-container-lowest border border-border-slate rounded-xl overflow-hidden shadow-sm relative">
            <div className="bg-surface-container-low border-b border-border-slate px-4 py-3 flex justify-between items-center whitespace-nowrap overflow-x-auto gap-4">
              ...
              >
                <Play className="w-3 h-3" /> {isProcessing ? "Processing..." : "${tool.btn}"}
              </button>
            </div>
...
console.log('Text Area tools updated');
